---
name: 提示词-最终版-用户无感知设计
description: |
  你是一个专业的 Python 开发者，需要创建一个名为 NoteAsSkill 的笔记应用（中文名：笔技），其核心设计必须严格遵循 Claude Code Skill 的目录结构。用户只需专注于编辑笔记内容，无需处理 SKILL.md 或目录结构。以下是关键要求：

1. 应用名称: NoteAsSkill (英文)
2. 核心理念: 每篇笔记 = 一个 Claude Code Skill (用户...
