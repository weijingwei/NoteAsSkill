...
class TestResult:
    ...
    def add(self, name: str, passed: bool, message: str = ""):
        ...
        try:
            ...
            print(f"  [{status}] {safe_name}: {safe_msg}")
    ...
    def summary(self):
        ...
        for name, status, message in self.results:
            try:
                ...
                safe_msg = message.encode('ascii', 'replace').decode('ascii')
                print(f"[{status}] {safe_name}: {safe_msg}")
        print(f"\nPass rate: {self.passed}/{total} ({self.passed/total*100:.1f}%)")
...
"""
        ...
        result.add("SKILL.md AI generation test", False, f"error: {str(e)}")
...
def test_ai_integration():
    ...
    try:
        ...
        result.add("AI integration test", False, f"encoding error: {str(e)[:50]}")
    except Exception as e:
...
def test_common_components():
    ...
    try:
        from app.widgets.common import NoBorderComboBox, load_icon, IconManager, get_icon_manager

        result.add("NoBorderComboBox 类", NoBorderComboBox is not None, "NoBorderComboBox defined")
        result.add("load_icon 函数", callable(load_icon), "load_icon defined")
        result.add("IconManager 类", IconManager is not None, "IconManager defined")
        result.add("get_icon_manager 函数", callable(get_icon_manager), "get_icon_manager defined")

        manager1 = get_icon_manager()
        manager2 = get_icon_manager()
        result.add("IconManager 单例模式", manager1 is manager2, "same instance")

        icon = load_icon("icon")
        result.add("load_icon 加载图标", not icon.isNull(), "icon loaded")

    except Exception as e:
        result.add("公共组件测试", False, f"error: {str(e)}")

    return result


def test_styles_module():
    """测试样式模块"""
    print("\n=== 测试样式模块 ===")
    result = TestResult()

    try:
        from app.widgets.styles import (
            Colors, FontSizes, ButtonStyles, InputStyles,
            ListStyles, ScrollBarStyles, get_color, get_label_style
        )

        result.add("Colors 类", Colors is not None, "Colors defined")
        result.add("Colors.PRIMARY", Colors.PRIMARY == "#D4A574", f"color={Colors.PRIMARY}")
        result.add("Colors.BACKGROUND", Colors.BACKGROUND == "#FFFEF9", f"color={Colors.BACKGROUND}")
        result.add("FontSizes 类", FontSizes is not None, "FontSizes defined")
        result.add("ButtonStyles 类", ButtonStyles is not None, "ButtonStyles defined")
        result.add("ButtonStyles.PRIMARY", len(ButtonStyles.PRIMARY) > 0, "has PRIMARY style")
        result.add("InputStyles 类", InputStyles is not None, "InputStyles defined")
        result.add("ListStyles 类", ListStyles is not None, "ListStyles defined")
        result.add("ScrollBarStyles 类", ScrollBarStyles is not None, "ScrollBarStyles defined")
        result.add("get_color 函数", callable(get_color), "get_color defined")
        result.add("get_label_style 函数", callable(get_label_style), "get_label_style defined")

        style = get_label_style(14, Colors.TEXT, True)
        result.add("get_label_style 生成样式", "font-size" in style and "bold" in style, f"style={style[:30]}...")

    except Exception as e:
        result.add("样式模块测试", False, f"error: {str(e)}")

    return result


def test_ai_defaults_module():
    """测试 AI 默认配置模块"""
    print("\n=== 测试 AI 默认配置模块 ===")
    result = TestResult()

    try:
        from app.core.ai_defaults import AIDefaults

        result.add("AIDefaults 类", AIDefaults is not None, "AIDefaults defined")

        result.add("OPENAI 配置", "base_url" in AIDefaults.OPENAI, f"url={AIDefaults.OPENAI['base_url']}")
        result.add("ANTHROPIC 配置", "base_url" in AIDefaults.ANTHROPIC, f"url={AIDefaults.ANTHROPIC['base_url']}")
        result.add("OLLAMA 配置", "base_url" in AIDefaults.OLLAMA, f"url={AIDefaults.OLLAMA['base_url']}")

        openai_url = AIDefaults.get_base_url("openai")
        result.add("get_base_url('openai')", openai_url == "https://api.openai.com/v1", f"url={openai_url}")

        openai_model = AIDefaults.get_default_model("openai")
        result.add("get_default_model('openai')", openai_model == "gpt-4", f"model={openai_model}")

        providers = AIDefaults.all_providers()
        result.add("all_providers()", "openai" in providers and "anthropic" in providers and "ollama" in providers, f"providers={providers}")

    except Exception as e:
        result.add("AI 默认配置测试", False, f"error: {str(e)}")

    return result


def test_workers_module():
    """测试 Worker 基类模块"""
    print("\n=== 测试 Worker 基类模块 ===")
    result = TestResult()

    try:
        from app.core.workers import BaseWorker, WorkerManager, get_worker_manager

        result.add("BaseWorker 类", BaseWorker is not None, "BaseWorker defined")
        result.add("WorkerManager 类", WorkerManager is not None, "WorkerManager defined")
        result.add("get_worker_manager 函数", callable(get_worker_manager), "get_worker_manager defined")

        manager1 = get_worker_manager()
        manager2 = get_worker_manager()
        result.add("WorkerManager 单例模式", manager1 is manager2, "same instance")

        result.add("BaseWorker.finished 信号", hasattr(BaseWorker, 'finished'), "finished signal exists")
        result.add("BaseWorker.progress 信号", hasattr(BaseWorker, 'progress'), "progress signal exists")

        result.add("WorkerManager.start_worker", callable(manager1.start_worker), "start_worker method exists")
        result.add("WorkerManager.cancel_worker", callable(manager1.cancel_worker), "cancel_worker method exists")
        result.add("WorkerManager.cancel_all", callable(manager1.cancel_all), "cancel_all method exists")

    except Exception as e:
        result.add("Worker 基类测试", False, f"error: {str(e)}")
...
def test_version_management():
    ...
    try:
        ...
        version = get_version()
        result.add("get_version() 返回值", version.startswith("v"), f"version={version}")

        sys_config = get_system_config_instance()
        result.add("SystemConfig.version", sys_config.version == version, f"version={sys_config.version}")

        import re
        version_pattern = r'^v\d+\.\d+\.\d+$'
        result.add("版本号格式正确", bool(re.match(version_pattern, version)), f"format={version}")

    except Exception as e:
        result.add("版本号管理测试", False, f"error: {str(e)}")
...
def main():
    ...
    all_results.append(test_common_components())
    all_results.append(test_styles_module())
    all_results.append(test_ai_defaults_module())
    all_results.append(test_workers_module())
    all_results.append(test_version_management())
